...
class IdleProcess(BasicStimulationProcess):
    ...
    def _idleInterface(self):
        ...
        prepare_finish_text = visual.TextStim(self.w, pos=[0, 0], text=text, color=(255, 255, 255),
                                 colorSpace='rgb255')
    ...
    def update(self):
        ...
        # update cue
        if self.exchange_message_management.exchange_message_operator.state_monitor.trainEnd is 'FreeSpelling':
            text = ' >>'
            # free spelling 的标记是66？
            self.controller.blockCues = np.tile(60,self.targetNUM)
        else:
