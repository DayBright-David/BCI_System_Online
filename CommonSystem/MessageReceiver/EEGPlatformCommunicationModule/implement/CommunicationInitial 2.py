...
from CommonSystem.MessageReceiver.EEGPlatformCommunicationModule.interface.CommunicationInitialInterface import CommunicationInitialInterface
from confluent_kafka.admin import AdminClient, NewTopic
import json
import os
import time
from CommonSystem.MessageReceiver.EEGPlatformCommunicationModule.exception import Exceptions
...
class CommunicationInitial(CommunicationInitialInterface):
    ...
    def topicCreate(topic, confPath, num_partitions=1, replication_factor=1):
        ...
        if not "strap.servers" in conf.keys():
